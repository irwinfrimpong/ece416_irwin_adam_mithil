...
module wf_pkt_tb (
    ...
    class wf_pkt;
        ...
        constraint c_err_type { err_type dist { NO_ERROR:=100, CRC_ERROR:=0}; }

        constraint c_dst { dst dist {"$":=10, "5":=20, "3":=10, "*":=60};}
        // only try type 0 and 1 packets for now
        //constraint c_pkt_type { pkt_type inside { ["0":"1"] }; }
        constraint c_pkt_type { pkt_type == "0"; }
        ...
        task send_pkt_mx;
            ...
            send_byte(src);
            send_byte(pkt_type);
            for (int i=0; i < data.size(); i++) begin  // send all but the last byte
                send_byte(data[i]);
            end
