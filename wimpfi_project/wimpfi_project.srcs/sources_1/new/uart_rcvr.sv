...
module uart_rcvr(
    ...
    controller_uart_rcvr UARTRCVR_FSM_CNTRL(.clk(clk), .rst(rst), .rdy(rdy), .rxd(rxd), .br_en(br_en),.ct_initeq(ct_initeq), .br_2en(br_2en), .ct_eq(ct_eq),.valid(valid),.oerr_cntrl(oerr_cntrl),
    ...
    datapath_uart_rcvr #(.BAUD_RATE(BAUD_RATE)) UARTRCVR_DATAPATH(.clk(clk), .rst(rst),.set_ferr(set_ferr), .clr_ferr(clr_ferr), .set_oerr(set_oerr), .clr_oerr(clr_oerr), .set_valid(set_valid), .clr_valid(clr_valid),
